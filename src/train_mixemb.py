...
from src.spk_emb import Mel, SpeechEmbedder
from src.spkemb_tasnet import SpkEmbTasNet
...
class Trainer(Solver):
    ...
    def set_model(self):
        self.model = SpkEmbTasNet(self.config['model']).to(DEV)
        ...
        embedder.eval()
        self.embedder = nn.Sequential(mel_ext, embedder).to(DEV)
    ...
    def train_one_epoch(self, epoch, tr_loader):
        ...
        for i, sample in enumerate(tqdm(tr_loader, ncols = NCOL)):
            ...
            with torch.no_grad():
                for b in range(B):
                    wav = padded_mixture[b, :mixture_lengths[b]]
                    dvec = self.embedder(wav)
                    dvecs.append(dvec)
            dvecs = torch.stack(dvecs, dim = 0)

            estimate_source = self.model(padded_mixture, dvecs)
