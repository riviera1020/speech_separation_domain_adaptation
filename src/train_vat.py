...
class Trainer(Solver):
    ...
    def __init__(self, config, stream = None):
        ...
        self.xi = config['solver']['xi']
        self.eps = config['solver']['eps']
        self.vat_iters = config['solver']['vat_iters']
    ...
    def set_model(self):
        ...
        self.Lv_scheduler = self.set_scheduler(self.config['solver']['Lv_scheduler'])
    ...
    def vat_loss(self, ul_mix, ul_len, xi, eps, num_iters):

        # gen r_adv
        with torch.no_grad():
            ul_sep = self.model(ul_mix)

        d = ul_mix.clone().detach().normal_()
        for i in range(num_iters):
            d = xi * self.l2_normalize(d)
            d.requires_grad_()
            pert_sep = self.model(ul_mix + d)

            loss, max_snr, estimate_source, reorder_estimate_source = \
                cal_loss(ul_sep, pert_sep, ul_len)
            loss.backward()
        ...
        # cal loss
        adv_sep = self.model(ul_mix + r_adv)
        loss, max_snr, estimate_source, reorder_estimate_source = \
            cal_loss(ul_sep, adv_sep, ul_len)
        return loss
    ...
    def train_one_epoch(self, epoch, sup_loader, uns_gen):
        ...
        for i, sample in enumerate(tqdm(sup_loader, ncols = NCOL)):
            ...
            vat_loss = self.vat_loss(padded_mixture, mixture_lengths, self.xi, self.eps, self.vat_iters)
            ...
            vlambda = self.Lv_scheduler.value(epoch)
            loss = sup_loss + vlambda * vat_loss
            ...
            total_vat += vat_loss.item()

            self.step += 1

        total_loss = total_loss / len(sup_loader)
        total_vat = total_vat / len(sup_loader)
        self.writer.add_scalar('train/epoch_loss', total_loss, epoch)
        self.writer.add_scalar('train/epoch_vat_loss', total_vat, epoch)
